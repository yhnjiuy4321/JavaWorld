public class CarIdUtil {
  ...
  public static boolean isValidCarId(String carId) throws InvalidCarIdException {
    if(carId.matches("C[A-Z]{2}-\\d{4}")) {
      return true;
    }
    throw new InvalidCarIdException("Car id format is not correct: " + carId);
  }

  public static boolean isValidTaxiId(String carId) throws InvalidCarIdException {
    if(carId.matches("T[A-Z]{2}-\\d{3}")) {
      return true;
    }
    throw new InvalidCarIdException("Taxi id format is not correct: " + carId);
  }

  public static boolean isValidElectricCarId(String carId) throws InvalidCarIdException {
    if(carId.matches("E[A-Z]{2}-\\d{4}")) {
      return true;
    }
    throw new InvalidCarIdException("Electric car id format is not correct: " + carId);
  }
